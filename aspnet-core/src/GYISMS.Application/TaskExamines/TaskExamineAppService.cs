...
namespace GYISMS.TaskExamines
{
    ...
    {
    private readonly IRepository<TaskExamine, int>
    _taskexamineRepository;
    ...
    /// <summary>
        /// 构造函数 
        ///</summary>
    public TaskExamineAppService(
    IRepository<TaskExamine, int>
...
taskexamineListDtos
		...
		/// </summary>
		public async Task<TaskExamineListDto> GetTaskExamineByIdAsync(EntityDto<int> input)
		{
...
taskexamineEditDto = new TaskExamineEditDto();
		...
		/// <param name="input"></param>
		/// <returns></returns>
		public async Task CreateOrUpdateTaskExamine(CreateOrUpdateTaskExamineInput input)
		{

			if (input.TaskExamine.Id.HasValue)
			{
				await UpdateTaskExamineAsync(input.TaskExamine);
			}
			else
			{
				await CreateTaskExamineAsync(input.TaskExamine);
			}
		}
		...
		/// 新增TaskExamine
		/// </summary>
		[AbpAuthorize(TaskExamineAppPermissions.TaskExamine_Create)]
		protected virtual async Task<TaskExamineEditDto> CreateTaskExamineAsync(TaskExamineEditDto input)
		{
			...
			entity = await _taskexamineRepository.InsertAsync(entity);
		...
		/// 编辑TaskExamine
		/// </summary>
		[AbpAuthorize(TaskExamineAppPermissions.TaskExamine_Edit)]
		...
		/// <summary>
		/// 导出TaskExamine为excel表,等待开发。
		/// </summary>
		/// <returns></returns>
		//public async Task<FileDto> GetTaskExaminesToExcel()
		//{
